#!/usr/bin/env python3
"""
Create a more compact teams grid by scaling down the team images.
"""
...
def create_compact_teams_grid():
    """Create a compact grid layout of all team images"""
    
    # Configuration
    rows = 25
    cols = 4
    scale_factor = 0.6  # Scale down images to 60% of original size
    
    # Load a sample team image to get dimensions
    sample_team = Image.open('results/teams/1.png')
    original_width, original_height = sample_team.size
    
    # Calculate scaled dimensions
    team_width = int(original_width * scale_factor)
    team_height = int(original_height * scale_factor)
    ...
    canvas = Image.new('RGBA', (canvas_width, canvas_height), color=(25, 25, 25, 255))
    ...
    # Draw header text
    title = "OPTC Rumble Teams - All Defensive Teams (Compact View)"
    now = datetime.datetime.now()
    subtitle = f"Generated on {now.strftime('%B %d, %Y')} - Top 99 Teams"
    ...
    draw.text((subtitle_x, margin + 35), subtitle, fill="lightgray", font=subtitle_font)
        ...
        for col in range(cols):
            ...
            if os.path.exists(team_file):
                try:
                    ...
                    scaled_img = team_img.resize((team_width, team_height), Image.Resampling.LANCZOS)
                    ...
                    canvas.paste(scaled_img, (x, y), scaled_img if scaled_img.mode == 'RGBA' else None)
                    teams_placed += 1
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            x = margin + (col * team_width) + 8
            y = margin + header_height + (row * team_height) + 8
            
            # Draw team number with background
            number_text = str(team_number)
            try:
                number_font = ImageFont.truetype("/System/Library/Fonts/Supplemental/Arial Bold.ttf", 14)
            except:
    ...
    output_file = os.path.join(output_dir, f'all_teams_compact_{now.strftime("%Y_%m_%d")}.png')
    canvas.save(output_file)
    
    print(f"Compact teams grid saved to: {output_file}")
    print(f"Final image size: {canvas.size}")
    
    return output_file

def main():
    print("Creating compact teams grid layout...")
    output_file = create_compact_teams_grid()
    print("Done!")
